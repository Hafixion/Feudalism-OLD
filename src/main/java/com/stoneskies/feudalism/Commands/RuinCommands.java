                        ...
                        if (resident.hasTown()) {
                            try {
                                ...
                                if (RuinAPI.isRuined(resident.getTown())) {
                                    ...
                                    resident.getPlayer().sendMessage(ChatInfo.msg("&6[Feudalism]&7 Town reclaimed, lead it into a better era."));
                                    Bukkit.broadcastMessage(ChatInfo.msg("&6[Feudalism]&7 " + resident.getName() + " has reclaimed " + resident.getTown().getName()));
                                } else {
                                    resident.getPlayer().sendMessage(ChatInfo.msg("&6[Feudalism]&7 Your town isn't ruined."));
                                }
                            } catch (NotRegisteredException e) {
                                e.printStackTrace();
                                resident.getPlayer().sendMessage(ChatInfo.msg("&6[Feudalism]&7 You aren't in a town."));
                            }
                        } else {
                            resident.getPlayer().sendMessage(ChatInfo.msg("&6[Feudalism]&7 You aren't in a town."));
                        }
                    }
                } else {sender.sendMessage(ChatInfo.msg("&6[Feudalism]&7 resident reclamation is disabled."));}
